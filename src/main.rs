...
async fn main() {
    ...
    // Calling the lib's register function informs the turbo engine of all macro-decorated values
    // and functions in our crate. This boilerplate is unfortunately necessary, but only needs to
    // be done in the root of the main function.
    ...
    // the task.
    loop {
        let (duration, _) = tt.get_or_wait_update_info(Duration::from_millis(100)).await;
        println!("updated computation done in {}", duration.as_millis());
    }
}
